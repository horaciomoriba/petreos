// src/pages/revisiones/NuevaRevision.jsx
...
const NuevaRevision = () => {
  ...
  const [licenciaVigente, setLicenciaVigente] = useState(true);
  ...
  const loadData = async () => {
    try {
      ...
      // Cargar vehículo
      const vehiculoData = await vehiculoService.getById(vehiculoId);
      setVehiculo(vehiculoData);

      // Cargar tipo de revisión según frecuencia
      const tipoRevisionData = await tipoRevisionService.getByVehiculoYFrecuencia(
        vehiculoData.tipo_vehiculo,
        frecuencia
      );
      setTipoRevision(tipoRevisionData);

      // Inicializar items PRE-MARCADOS como "bien"
      const todasLasPreguntas = [];
      if (Array.isArray(tipoRevisionData?.secciones)) {
        tipoRevisionData.secciones.forEach(seccion => {
      ...
      const comentariosSecciones = tipoRevisionData?.secciones?.map(seccion => ({
      ...
      if (vehiculoData?.configuracion_neumaticos?.configurado) {
        vehiculoData.configuracion_neumaticos.ejes.forEach(eje => {
      ...
      setFormData(prev => ({
        ...prev,
        datosOperacionales: {
          kilometraje: vehiculoData?.kilometraje_actual || '',
          horas_motor: vehiculoData?.horas_motor_actual || '',
          nivel_combustible: ''
        },
      ...
      showToast.error(error.response?.data?.message || 'Error al cargar datos');
      navigate(-1);
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        tipo_revision_id: tipoRevision._id,
        operador: {
          user_id: user._id,
          nombre: user.nombre,
          licencia_vigente: licenciaVigente
        },
        respuestas: formData.items.map(item => ({
          numero: item.numero,
          texto_pregunta: item.nombre,
          respuesta: item.estado === 'bien' ? 'Bien' : 'Mal',
          observaciones: item.observaciones || ''
        })),
        comentarios_secciones: formData.comentariosSecciones.filter(cs => cs.comentario.trim() !== ''), // 🆕 NUEVO
        llantas: formData.neumaticos, // 🆕 Ya incluye los nuevos campos
        comentarios_neumaticos: formData.comentariosNeumaticos, // 🆕 NUEVO
        comentarios: formData.comentarios,
        nivel_combustible: formData.datosOperacionales.nivel_combustible,
        kilometraje_al_momento: parseFloat(formData.datosOperacionales.kilometraje),
        horas_motor_al_momento: parseFloat(formData.datosOperacionales.horas_motor)
      };

      const response = await revisionService.createAdmin(payload);

      showToast.success('Revisión creada exitosamente');
      navigate(`/revisiones/${response.revision._id}`);
  ...
  const calcularProgreso = () => {
    ...
    const totalSteps = 1 + (tipoRevision?.secciones?.length || 0) + (vehiculo?.configuracion_neumaticos?.configurado ? 1 : 0);
    ...
    if (vehiculo?.configuracion_neumaticos?.configurado) {
      completedSteps++;
    }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-white flex items-center justify-center p-6">
        <div className="text-center">
          ...
          <h3 className="text-lg font-semibold text-gray-900 mb-2">Checklist no disponible</h3>
          <p className="text-sm text-gray-600 mb-8">No existe checklist {frecuencia} para este tipo de vehículo</p>
          <button 
            onClick={() => navigate(-1)}
          ...
          <div className="hidden lg:block lg:col-span-1">
            <div className="sticky top-24 space-y-4">
              ...
              <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
                <h3 className="text-sm font-semibold text-gray-900 mb-4">Tipo de revisión</h3>
                <div className="flex flex-col gap-2">
                  {['diaria', 'mensual', 'bimestral'].map(freq => (
                    <button
                      key={freq}
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                >
                  <div className="px-4 sm:px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-orange-50 to-transparent">
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">
                      {vehiculo.configuracion_neumaticos.ejes.reduce((acc, eje) => acc + eje.neumaticos.length, 0)} neumáticos en total
                    </p>
                  </div>
              ...
              <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                <div className="px-4 sm:px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-gray-50 to-transparent">
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">Opcional - Observaciones adicionales de la revisión completa</p>
                </div>
                <div className="p-4 sm:p-6">
                  <textarea
                    ...
                    rows="4"
                    placeholder="Agrega observaciones generales, situaciones especiales, o cualquier detalle relevante sobre toda la revisión..."
                  ></textarea>
                </div>
              </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-yellow-700">
                        Se encontraron {itemsMal + neumaticosProblema} problema(s). Esta revisión requerirá aprobación administrativa.
                      </p>
                    </div>
                  </div>
                </div>
