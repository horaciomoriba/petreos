// pages/reparaciones/EditarReparacion.jsx
...
const EditarReparacion = () => {
  ...
  const loadReparacion = async () => {
    try {
      ...
      setFormData({
        vehiculo_id: reparacion.vehiculo?._id || reparacion.vehiculo || '',
        categoria: reparacion.categoria,
        fecha_realizacion: reparacion.fecha_realizacion?.split('T')[0] || '',
        descripcion: reparacion.descripcion,
        notas_adicionales: reparacion.notas_adicionales || '',
        piezas: reparacion.piezas || [],
        costo_mano_obra: reparacion.costo_mano_obra || 0,
        estado: reparacion.estado,
        kilometraje_al_momento: reparacion.kilometraje_al_momento || '',
        horas_motor_al_momento: reparacion.horas_motor_al_momento || ''
      });

      // Cargar info del vehículo si está disponible
      if (reparacion.vehiculo?._id || reparacion.vehiculo) {
        const vehiculoId = reparacion.vehiculo?._id || reparacion.vehiculo;
        const vehiculoResponse = await vehiculoService.getById(vehiculoId);
        const vehiculoData = vehiculoResponse.vehiculo || vehiculoResponse.data || vehiculoResponse;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await reparacionService.update(id, dataFormateada);
      showToast.success('Reparación actualizada exitosamente');
      navigate(`/reparaciones/${id}`);
    } catch (error) {
      showToast.error(error.response?.data?.message || 'Error al actualizar reparación');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="p-4 lg:p-8">
      ...
      <div className="mb-6">
        <div className="flex items-center gap-3 mb-2">
          <button
            onClick={() => navigate(`/reparaciones/${id}`)}
          ...
          <div>
            <h1 className="text-2xl font-bold text-primary-900">Editar Reparación</h1>
            <p className="text-sm text-primary-600">Actualiza la información de la reparación</p>
          </div>
        </div>
      </div>
